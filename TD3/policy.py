...
class BasePolicyNet(nn.Module, Policy):
    ...
    def noisy_forward(self, x: Tensor, action_low: Tensor | None, action_high: Tensor | None) -> Tensor:
        next_actions = self(x)
        noise = (torch.randn_like(next_actions) * POLICY_NOISE).clamp(-NOISE_CLAMP, NOISE_CLAMP) # I worry the clamping will cause unde accumulation at the edge of the range
        next_actions = (next_actions + noise)
        if (action_low is not None) and (action_low is not None):
            next_actions = next_actions.clamp(action_low, action_high)
        return next_actions
...
class StabilisingPolicyNet(BasePolicyNet):
    ...
    def deterministic_update(self, critic_net: StabilisingCriticNet, state_batch: Tensor):
