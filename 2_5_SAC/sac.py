...
class SAC:
    ...
    def update(self, replay_memory: ReplayMemory):
        for step in range(self._n_steps):
            ...
            critic_loss_1, critic_loss_2 = self._update_critic(batch)
    ...
    def _update_actor(self, batch: Transition) -> float:
        ...
        #with self._critic.freeze():
            # note that the previous action forms part of the state, so we pass it along with the observation
        q = self._critic.get_value(observations, actions, target=False)
    ...
    def _update_critic(self, batch: Transition):
        observations: Tensor = batch.state
        next_observations: Tensor = batch.next_state
        rewards: Tensor = batch.reward
        actions: Tensor = batch.action
        done: Tensor = batch.done

        backup = self._get_backup(next_observations, rewards, done)

        loss_1, loss_2 = self._critic.update(observations, actions, backup)
